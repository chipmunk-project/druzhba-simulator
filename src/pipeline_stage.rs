...
impl PipelineStage {
    ...
    pub fn with_alus (stateless: Vec <ALU>, 
                      stateful: Vec<ALU>, 
    ...
    pub fn tick(&mut self, 
        ...
        if input_phv.is_bubble() {
          ...
          for alu in self.stateful_alus.iter_mut () {
              ...
              let mut phv_con_operands = alu.input_mux_output();
              let state_result = alu.run (&mut phv_con_operands);
          ...
          for alu in self.stateless_alus.iter_mut() {
              ...
              alu.send_packets_to_input_muxes(input_phv.clone());
              let phv_con_operands = alu.input_mux_output();
              //After being passed to alu, value is sent to an
              //output mux and put into a PHV
  
              let result =  alu.run(&phv_con_operands).0[0];
              // State variables and returned value from stateless ALU
              let mut output_mux_fields = state_for_output_mux.clone();
