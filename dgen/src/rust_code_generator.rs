  ...
  fn fmt (&self, f : &mut fmt::Formatter) -> fmt::Result {
    match &*self {
      Alu::Program (opt_header, header, stmt) => {
        ...
        let mut outer_header = String::from("pub fn ");
  ...
  fn fmt (&self, f: &mut fmt::Formatter) -> fmt::Result {
    ...
    match &*self{
      ...
      Stmt::If (e1, if_block, elif_block, else_block) => {
          ...
          let mut elif_bodies = String::from("");
          ...
          if is_stateful || optimize == 0  || !(expr.contains("opcode") && expr.contains("hole_vars")) {
              ...
              else_body= match else_block {
                Some(else_block_stmts) => {
                  let mut tmp_str = String::from("        else{\n");
          ...
          // Optimized stateless ALU
              ...
              let name = (&*NAME.read().unwrap()).to_string();
...
impl fmt::Display for Expr {
  fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
    ...
    match &*self{
      ...
      Expr::Opt (e) => {
          let opt_hole_name = generate_hole_name ("Opt".to_string());
          let opt_name = generate_helper_name ("Opt".to_string());
          if optimize == 0 {
      ...
      Expr::ArithOp (e1, e2) => {
        ...
        if optimize == 0{
          ...
          write! (f, "{} ({}, {}, hole_vars[\"{}\"])", 
           ...
           let opcode = match HOLE_VALS
                .read()
                .unwrap()
                .get(&arith_op_hole_name) {
                 Some (num) => *num,
      ...
      Expr::Relop (e1, e2) => {
        let rel_op_hole_name = generate_hole_name ("rel_op".to_string());

        let rel_op_name = generate_helper_name ("rel_op".to_string());
        if optimize == 0{
...
fn generate_helper_name (helper_name: String) -> String {

     let helper_num = FUNC_COUNT
        .read() 
        .unwrap() [&helper_name];  
     let pipeline = *PIPELINE_STAGE.read().unwrap();
     let alu = *ALU_NUMBER.read().unwrap();
...
pub fn extract_hole_cfgs (hole_cfgs_file: String) -> HashMap <String, i32> {
  ...
  let hole_cfgs_file_contents = fs::read_to_string(hole_cfgs_file).expect ("Error: Hole configs file could not be found");
...
fn generate_rel_op (rel_op_name: String)
{
  ...
  let if_ret = String::from
      ("  if opcode == 0 {\n    (op1 != op2) as i32\n  }\n");
  let elif_ret_1 = String::from
      ("  else if opcode == 1{\n    (op1 < op2) as i32\n  }\n");
  let elif_ret_2 = String::from
      ("  else if opcode == 2{\n    (op1 > op2) as i32\n  }\n");
  let else_ret = String::from
      ("  else{\n    (op1 == op2) as i32\n  }\n}\n");
  let mut rel_op_fn = String::from("");
...
fn generate_arith_op_optimized (arith_op_name: String,
                                opcode: i32) {
  let fn_header = format!("fn {}(op1 : i32, op2 : i32) -> i32{{\n",
      arith_op_name);
  let fn_body = 
      match opcode {
...
fn generate_arith_op (arith_op_name: String)
{
  ...
  let else_ret = String::from 
      ("  else {\n  op1 - op2\n  }\n}\n");
  let mut arith_op_fn = String::from("");
...
fn generate_opt_optimized (opt_name: String,
                           opcode: i32) {
  let fn_header = format!("fn {}(op : i32) -> i32{{\n", 
      opt_name);
  let fn_body = 
      match opcode {
        0 => String::from("  op\n}\n"),
